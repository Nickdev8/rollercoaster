...
@export var resolution:      float   = 50.0    # desired pixels per segment and input threshold
@export var min_segments:    int     = 5       # minimum number of smoothing segments
@export var min_seg_width:   float   = 8.0     # minimum segment width in pixels
...
func _input(event):
	# adaptive input threshold based on resolution
		...
		match event.keycode:
			...
			KEY_R:
				for j in range(carts.size()):
					carts[j].offset = j * cart_spacing
					carts[j].speed = initial_speed
...
func get_track_points() -> PackedVector2Array:
	...
	var auto_segs = int(length / resolution)
	var max_segs  = int(length / min_seg_width)
	var segs      = clamp(auto_segs, min_segments, max_segs)
